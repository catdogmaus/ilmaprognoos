...
class IlmaprognoosBaseSensor(CoordinatorEntity, SensorEntity):
    ...
    def available(self) -> bool:
        # --- FIX: Ensure sensors don't go 'Unavailable' during a background network glitch ---
...
class IlmaprognoosWarningsSensor(IlmaprognoosBaseSensor):
    ...
    def state(self):
        ...
        # --- FIX: Smarter truncation logic for single, very long warnings ---
        first_warning = descriptions[0]
        remaining_count = len(descriptions) - 1
        
        if remaining_count > 0:
            suffix = f"\n\n... ja {remaining_count} hoiatus." if remaining_count == 1 else f"\n\n... ja {remaining_count} hoiatust."
        else:
            suffix = ""
...
# ... [All other sensors (Precipitation, Temperature, Humidity, Water, Sunshine, Forecasts) remain exactly identical to before] ...
...
class IlmaprognoosSunshineDurationSensor(IlmaprognoosBaseSensor):
    _attr_name = "Päikesepaiste kestus (mõõdetud)"; _attr_native_unit_of_measurement = UnitOfTime.HOURS; _attr_icon = "mdi:timer-sand"; _attr_state_class = SensorStateClass.TOTAL_INCREASING
    def __init__(self, coordinator):
